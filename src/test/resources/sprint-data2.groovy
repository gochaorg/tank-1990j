def SpriteData( int frameCount, double duration, int left, int bottom, int width, int height, boolean loop, String ignoreArg ){
    [ frames: frameCount, duration: duration,
      left: left, top: bottom - height,
      width: width, height: height,
      loop: loop
    ]
}
sprites = [:]
def Insert( String name, def data ){
    def m = [:]
    m.putAll(data)
    sprites[ name ] = m
}
...
public class SpriteData {
    ...
    public SpriteData( int fc, double dur, int lf, int tp, int w, int h, boolean l ){
        this.frames = fc;
        this.duration = dur;
        this.left = lf;
        this.top = tp;
...
sprites.each {name, s ->
    idx++;
    println "  public static final SpriteData $name = new SpriteData( $s.frames, $s.duration, $s.left, $s.top, $s.width, $s.height, $s.loop );"
}
println "}"